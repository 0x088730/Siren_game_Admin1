...
const PresalePage = ({ setPage }: Props) => {
  const dispatch = useDispatch();
  const accessToken = useSelector(selectAccessToken)
  const showData: PresaleDataType[] = useSelector(
    (state: RootState) => state.referData.data
  );
  useEffect(() => {
    dispatch(referData(accessToken) as unknown as AnyAction);
  }, [accessToken]);
  return (
    <div className="pt-24">
      <Box
