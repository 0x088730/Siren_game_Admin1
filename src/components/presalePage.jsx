        ...
        {nav === "transaction" ?
          ...
          <div className="w-full flex justify-center items-center flex-col">
            {showUserList.map(({ walletAddress, Ref, usdt, guest }, index) => (
              <div key={index} className="border-b-2 border-black my-[10px] w-[768px]">user #{index + 1} | wallet: {walletAddress} | Ref: <span className="text-[#35ee5a] font-bold text-[20px] cursor-pointer" onClick={(e) => modalOpen(guest, e)}>{Ref}</span>  Earned usdt: {usdt}</div>
            ))}
          </div>
        }
